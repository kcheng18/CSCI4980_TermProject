...
public class MyGraphView {
   ...
   private void addSelectionListenerMenuItemOpenNodeInNewView() {
	   SelectionListener menuItemListenerOpenNodeInNewView = new SelectionListener() {
		   @Override
	       public void widgetSelected(SelectionEvent e) {
			   System.out.println("[DBG] MenuItem OpenNodeInNewView");
			   if (!isNodesSelected()) {
	               String msg = "Please select a method to open a new view";
	               UtilMsg.openWarning(msg);
	               return;
	           }
			   DeclarationVisitor declVisitor = new DeclarationVisitor();
			   declVisitor.setOpen(true);
			   declVisitor.setMethodToBeOpened((GMethodNode) selectedGMethodNode);
			   syncZestViewAndJavaEditor();
			   declVisitor.setOpen(false);
			   selectedGMethodNode.setNodeType(GNodeType.InValid);
	       }
